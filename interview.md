...
2. 交互式系统：轮转调度、优先级调度、最短进程调度、彩票调度
3. 实时系统的调度：周期性事件流
...
1. `mmap`
   ...
   用来在**进程虚拟内存地址空间**中分配地址空间，创建和**物理内存的映射关系**

   注意：mmap 只是在虚拟内存分配了地址空间，只有在第一次访问虚拟内存的时候才分配物理内存(写时复制，**唯一的一次拷贝发生在这里**)

   普通的 read 系统调用，进程不能直接访问内核数据，需要转存。而 mmap 在用户态中直接访问内核数据，**直接少了一次复制过程，用内存读写代替 IO 读写**

   > 总的来说，mmap 是一种 IPC 方式，多进程下对彼此的修改可见，仅拷贝数据一次
...
- 分页与页面置换算法
  ...
  在 Linux 中，没有预分页和工作集的概念，是一个请求换页系统，换页到**交换区 swap**一个独立分区
...
- 内存模型
   ...
   JVM的多线程内存部分分为**主内存**与**工作内存**，主内存在堆上（类似于物理主内存），工作内存在每个线程（类似于高速缓存）
...
2. 原子性、可见性、有序性
   ...
   Java保证有序性通过volatile、synchronized和先行发生原则来完成
...
1. 内核线程实现 1:1
   ...
   内核线程->轻量级进程，Java用的是这种，需要进行用户/内核态的切换
...
2. 抢占式调度
   ...
   Java是抢占式调度，每个线程由**系统**分配时间

   Java代码层面只能给操作系统提“建议”（如`priority`/`yield`），但不能保证
...
- 协程
   ...
   原有1:1内核线程并发缺陷——内核/用户态的上下文切换代价极高

   > 注意不一定是协同式调度

   Java的协程还在发展，***Kotlin的协程还需继续研究***
...
1. 互斥同步——悲观锁
   ...
   synchronized和ReentrantLock，详情见下Java多线程
...
4. biased偏向锁、轻量级锁、重量级锁

   见下java多线程，**sync的锁升级过程**
...
`Mark Word` 运行时数据，在JVM堆中存储对象的**锁状态**、`Hashcode`、**GC分代年龄**等
...
1. 重量级锁不使用自旋，不消耗CPU，但是会直接进入阻塞态（**用户态到内核态的切换**），同步时间长

2. 轻量级锁线程不会阻塞，通过自旋同步，不过会消耗CPU，如果自旋失败会出现膨胀为重锁
...
- 为什么用B+树？
   ...
   **B+树的磁盘读写代价更低**，因为**非叶子节点不会存储像B树一样的具体信息的指针**，减少了存储压力

   B+树的叶子节点以**链表形式连接**，**范围查询的时候性能很高**（注意：**B树对数据库范围查询的支持效率极低**）
