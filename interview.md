...
5. **线程没有地址空间**，其包含在进程的地址空间中，一般来说，每个线程上下文包含一个**栈、寄存器、程序计数器**、以及调度相关的东西

> 崩溃一般伴随着**栈溢出、读取/访问非法地址**。**线程崩溃一般来说会导致进程崩溃**，因为共用同一个存储空间，无法确认会不会对其他线程造成影响，所以直接一起崩溃，但是在 JVM 中不会这样

#### 内核线程、轻量级进程、用户线程

1. 内核线程：内核的分身，内核线程使用廉价，唯一使用的资源是内核栈和上下文

2. 轻量级进程 LWP：**建立在内核之上由内核支持的用户线程**，每一个 LWP 都与一个**内核线程**对应，像**普通进程**一样被调度，Java 中是进行这样的对应的，所以会进行用户态和内核态的切换

3. 用户线程：完全建立在用户空间的线程库，**调度、同步等均在用户空间完成**

> 在 Linux 中，一个**用户线程**对应一个**轻量级进程**，而一个**轻量级进程**对应一个特定的**内核线程**
...
2. 解析与分派
   ...
   Java 是**静态多分派&动态单分派**的类型

   解析——前期绑定，编译期确定

   分派——后期绑定，运行时确定

   - 静态分派：**重载**取决于静态类型调用，不同重载调用不同的参数
...
- final 与闭包
  ...
  匿名内部类来自外部闭包环境的自由变量**必须是 final** 的，编译器会在匿名内部类中做处理，使得不能修改外部参数的值

  主要原因是匿名内部类类似于普通调用方法，在参数的传递中，基本类型是拷贝的，而引用类型也不可修改，只能修改其对象，所以为了在代码层面上强调这个问题，加入了 final 参数
...
1. px（分辨率）
2. dpi（像素密度）= 屏幕大小（px）/屏幕大小（英寸），每英寸有多少像素点
3. dp（密度无关像素）

   dp与px的转换与dpi密切相关

   > **默认160dpi、320*480下，1dp=1px**；其他dpi按照比例进行px的换算

4. sp（独立比例像素）

   用此单位设置文字大小，推荐偶数大小，防止精度丢失
...
1. 布局适配
   ...
   根据屏幕配置加载对应的UI布局，使用限定符

   限定符分为（手机or平板）最小宽度限定符（`sw xxxdp`）、布局别名（`<resource>标签`）、屏幕方向限定符

2. 布局组件匹配

   wrap_content、match_parent、layout_weight 等自适应组件尺寸

3. 图片资源适配

   9-patch图，自动拉伸位图（如聊天框）
...
3. 字节跳动适配方案，修改density，以**宽度或者高度为基准**
4. 图片资源的适配

   缺点：apk体积会变得很大，美工工作量也变大

   解决：唯一一套分辨率规格的图片资源xhdpi 320dpo
   （Iphone6标准）
...
- 整体流程
  ...
  先尝试命中缓存，如果没有则生成对象并放入缓存，如果命中则需判断是否为**多进程模式**，如果有则重新**读取文件**，仅此而已！
